...
const ROLE_VALUES = [ROLE_USER, ROLE_SELLER, ROLE_ADMIN];
...
function normalizeRole(role) {
  const normalized = String(role || ROLE_USER).trim().toLowerCase();
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!normalizedRole) {
    return res.status(400).json({
      error: 'role must be one of: user, seller, admin',
    });
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  return res.status(200).json({ accessToken, refreshToken, user: toPublicUser(user) });
});
...
function roleRoutePayload(req, message) {
  return {
    ok: true,
    message,
    role: req.user.role,
...
app.get(
  ['/api/product/role', '/api/products/role'],
  authMiddleware,
  allowRoles(ROLE_SELLER, ROLE_ADMIN),
  (req, res) => {
    return res.status(200).json(
      roleRoutePayload(req, 'Маршрут доступен для ролей seller и admin')
    );
  }
);

app.get(
  ['/api/product/admin/role', '/api/products/admin/role'],
  authMiddleware,
  allowRoles(ROLE_ADMIN),
  (req, res) => {
    return res.status(200).json(roleRoutePayload(req, 'Маршрут доступен только admin'));
  }
);

app.post('/api/products', authMiddleware, allowRoles(ROLE_SELLER, ROLE_ADMIN), (req, res) => {
...
app.put('/api/products/:id', authMiddleware, allowRoles(ROLE_SELLER, ROLE_ADMIN), (req, res) => {
  const product = products.find((item) => item.id === req.params.id);
...
app.delete('/api/products/:id', authMiddleware, allowRoles(ROLE_ADMIN), (req, res) => {
  const product = products.find((item) => item.id === req.params.id);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Task11 server started at http://localhost:${PORT}`);
  });
}
