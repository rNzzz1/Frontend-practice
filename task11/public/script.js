...
function mapAuthError(status, data) {
  ...
  if (status === 400 && rawError.includes('role')) {
    return 'Выберите корректную роль: user, seller или admin';
  }
  if (status === 400) {
...
registerForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      showMessage(`Регистрация успешна. Роль: ${payload.role}. Теперь выполните вход.`, 'success');
      return;
    }
